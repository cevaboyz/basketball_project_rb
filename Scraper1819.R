...
url <- "https://fip-web.azurewebsites.net/PlayerStats.aspx?Type=T&id=208&rg=LO"

session <- bow(url, user_agent = "cevaboyz")

totalstat2019 <- scrape(session) %>% html_nodes("table.ElencoGiocatori")

test <- html_table(totalstat2019)
