...
url <- "https://fip-web.azurewebsites.net/PlayerStats.aspx?Type=T&id=21&rg=LO"

session <- bow(url, user_agent = "cevaboyz")

totalstat2017 <- scrape(session) %>% html_nodes("table.ElencoGiocatori")

test <- html_table(totalstat2017)
...
write.csv(test2, file = "totalstat2016_2017.csv")
