...
url <- "https://fip-web.azurewebsites.net/PlayerStats.aspx?Type=T&id=183&rg=LO"

session <- bow(url, user_agent = "cevaboyz")

totalstat2018 <- scrape(session) %>% html_nodes("table.ElencoGiocatori")

test <- html_table(totalstat2018)
...
test2 <- subset(test2, select = -1)
test2 <- test2[3:nrow(test2),]



write.csv(test2, file = "totalstat2017_2018.csv")
